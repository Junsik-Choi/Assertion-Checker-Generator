...
# ===== 플러그인 =====
@register
class VACTPlugin(BaseAssertionPlugin):
    plugin_name = "vact"
    sheet_name = "VACT"
    ...
    def parse(self, xls_path: Path) -> Dict[str, Any]:
        ...
        # VACT 시트 확인
        try:
            ws_vact = _get_sheet_ci(wb_w, self.sheet_name)
        except KeyError:
        ...
        # 필수 라벨 위치 찾기
        hs_row, hs_col = _find_cell(ws_vact, "Hsync Signal")
        vs_row, vs_col = _find_cell(ws_vact, "Vsync Signal")
        de_row, de_col = _find_cell(ws_vact, "Data Enable Signal")
        min_row, min_col = _find_cell(ws_vact, "Expected Min Value")
        max_row, max_col = _find_cell(ws_vact, "Expected Max Value")
        ...
        if (
            ...
            print(
                ...
                "not found in VACT sheet.",
                flush=True,
            )
            raise ValueError("Missing required VACT labels")

        # 기존 시트 값 읽기 (라벨 바로 아래 칸)
        hs_cell  = ws_vact.cell(row=hs_row  + 1, column=hs_col).value
        vs_cell  = ws_vact.cell(row=vs_row  + 1, column=vs_col).value
        de_cell  = ws_vact.cell(row=de_row  + 1, column=de_col).value
        min_cell = ws_vact.cell(row=min_row + 1, column=min_col).value
        max_cell = ws_vact.cell(row=max_row + 1, column=max_col).value
        ...
        # 인터랙티브 편집 루프 (HACT/HFP/HSW와 동일 UX, 항목만 5개)
        while True:
            print("\n==================== VACT Settings ====================")
        ...
        # 최종 선택 값을 시트에 기록
        ws_vact.cell(row=hs_row  + 1, column=hs_col,   value=hsync_signal)
        ws_vact.cell(row=vs_row  + 1, column=vs_col,   value=vsync_signal)
        ws_vact.cell(row=de_row  + 1, column=de_col,   value=data_enable_signal)
        ws_vact.cell(row=min_row + 1, column=min_col,  value=exp_min)
        ws_vact.cell(row=max_row + 1, column=max_col,  value=exp_max)

        # VACT 시트의 Base Clock/Reset 셀에도 Define에서 읽은 값 기록 (수식이 아닌 실제 값)
        clk_row, clk_col = _find_cell(ws_vact, "Base Clock")
        if clk_row:
            ws_vact.cell(row=clk_row, column=clk_col + 1, value=base_clk)

        rst_row, rst_col = _find_cell(ws_vact, "Base Reset")
        if rst_row:
            ws_vact.cell(row=rst_row, column=rst_col + 1, value=base_rst)
    ...
    def generate_sv(self, parsed: Dict[str, Any], context: Dict[str, Any]) -> List[str]:
        ...
        if not blocks:
            return ["// No VACT assertions generated.\n", ""]
        ...
        lines.append("")
        lines.append("int vact_de_cnt;")
        lines.append("logic l_vbp_vact_vfp;")
        lines.append("")
        ...
        lines.append(f"        if($fell({vsync_signal})) begin")
        lines.append("            l_vbp_vact_vfp <= 1;")
        lines.append("        end")
        lines.append(f"        else if($rose({vsync_signal})) begin")
        lines.append("            l_vbp_vact_vfp <= 0;")
        lines.append("        end")
        lines.append("        else begin")
        lines.append("            l_vbp_vact_vfp <= l_vbp_vact_vfp;")
        lines.append("        end")
        lines.append("    end")
        lines.append("end")
        lines.append("")
        ...
        lines.append("    end")
        lines.append("    else begin")
        lines.append(f"        if($rose({data_enable_signal})) begin")
        lines.append("            vact_de_cnt <= vact_de_cnt + 1;")
        lines.append("        end")
        lines.append(f"        else if($fell({data_enable_signal})) begin")
        lines.append("            vact_de_cnt <= 0;")
        lines.append("        end")
        lines.append("        else begin")
        lines.append("            vact_de_cnt <= vact_de_cnt;")
        lines.append("        end")
        lines.append("    end")
        lines.append("end")
        lines.append("")
        ...
        lines.append(f"    $fell(l_vbp_vact_vfp) |-> (({exp_min} <= vact_de_cnt) && (vact_de_cnt <= {exp_max}));")
        lines.append("endproperty")
        lines.append("")
        lines.append("assert property (p_vact) $display(\"[%0t] Assertion PASS\", $time);")
