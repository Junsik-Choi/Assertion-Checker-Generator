...
class HACTPlugin(BaseAssertionPlugin):
    ...
    def parse(self, xls_path: Path) -> Dict[str, Any]:
        ...
        # Hsync / Data Enable / Min / Max 값 초기화 및 셀 위치 확인
        ...
        if hs_row is None or de_row is None or min_row is None or max_row is None:
            print("ERROR: One or more required cells (Hsync / Data Enable / Expected Min / Expected Max) not found in HACT sheet.", flush=True)
            raise ValueError("Missing required HACT labels")

        # 기존 시트 값 읽고 없으면 candidates쓰거나 없으면 초기값
        hs_cell  = ws_hact.cell(row=hs_row  + 1, column=hs_col).value
        de_cell  = ws_hact.cell(row=de_row  + 1, column=de_col).value
        min_cell = ws_hact.cell(row=min_row + 1, column=min_col).value
        max_cell = ws_hact.cell(row=max_row + 1, column=max_col).value
        ...
        while True:
            ...
            if choice == "":
                ...
                if data_enable_signal in ("", "<Data Enable Signal>"):  missing.append("[2]")
                if exp_min            in ("", "<Expected Min Value>"):  missing.append("[3]")
                if exp_max            in ("", "<Expected Max Value>"):  missing.append("[4]")
            ...
            if choice == "1":
                ...
                try:
                    ...
                    min_val_int = 0
                exp_max = _pick_int_with_validation("Enter Expected Max Value", min_val=min_val_int)
            else:
    ...
    def generate_sv(self, parsed: Dict[str, Any], context: Dict[str, Any]) -> List[str]:
        ...
        # interface 코드 생성 (logic 포트를 가진 interface)
        lines: List[str] = []
        ...
        lines.append(f"    $fell({data_enable_signal}) |-> (({exp_min} <= hact_clk_cnt) && (hact_clk_cnt <= {exp_max}))")
    ...
    def write_to_excel(cls, excel_path: Path, data: Dict[str, Any], state: Optional[Any] = None) -> None:
        """Write HACT assertion data to Excel sheet using labeled row locations."""
        from openpyxl import load_workbook  # type: ignore
        
        try:
            wb = load_workbook(str(excel_path))
            
            # Find HACT sheet (case-insensitive)
            ...
            # Process first block (HACT typically has one block)
            block = blocks[0]
            ...
            raise RuntimeError(f"Failed to write HACT assertion to Excel: {str(e)}")