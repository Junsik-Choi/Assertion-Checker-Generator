...
# ===== 플러그인 =====
@register
class VSWPlugin(BaseAssertionPlugin):
    plugin_name = "vsw"
    sheet_name = "VSW"
    ...
    def parse(self, xls_path: Path) -> Dict[str, Any]:
        ...
        # VSW 시트 확인
        try:
            ws_vsw = _get_sheet_ci(wb_w, self.sheet_name)
        except KeyError:
        ...
        # 필수 라벨 위치 찾기
        hs_row, hs_col = _find_cell(ws_vsw, "Hsync Signal")
        vs_row, vs_col = _find_cell(ws_vsw, "Vsync Signal")
        de_row, de_col = _find_cell(ws_vsw, "Data Enable Signal")
        min_row, min_col = _find_cell(ws_vsw, "Expected Min Value")
        max_row, max_col = _find_cell(ws_vsw, "Expected Max Value")
        ...
        if (
            ...
            print(
                ...
                "not found in VSW sheet.",
                flush=True,
            )
            raise ValueError("Missing required VSW labels")

        # 기존 시트 값 읽기 (라벨 바로 아래 칸)
        hs_cell  = ws_vsw.cell(row=hs_row  + 1, column=hs_col).value
        vs_cell  = ws_vsw.cell(row=vs_row  + 1, column=vs_col).value
        de_cell  = ws_vsw.cell(row=de_row  + 1, column=de_col).value
        min_cell = ws_vsw.cell(row=min_row + 1, column=min_col).value
        max_cell = ws_vsw.cell(row=max_row + 1, column=max_col).value
        ...
        # 최종 선택 값을 시트에 기록
        ws_vsw.cell(row=hs_row  + 1, column=hs_col,   value=hsync_signal)
        ws_vsw.cell(row=vs_row  + 1, column=vs_col,   value=vsync_signal)
        ws_vsw.cell(row=de_row  + 1, column=de_col,   value=data_enable_signal)
        ws_vsw.cell(row=min_row + 1, column=min_col,  value=exp_min)
        ws_vsw.cell(row=max_row + 1, column=max_col,  value=exp_max)

        # VSW 시트의 Base Clock/Reset 셀에도 Define에서 읽은 값 기록 (수식이 아닌 실제 값)
        clk_row, clk_col = _find_cell(ws_vsw, "Base Clock")
        if clk_row:
            ws_vsw.cell(row=clk_row, column=clk_col + 1, value=base_clk)

        rst_row, rst_col = _find_cell(ws_vsw, "Base Reset")
        if rst_row:
            ws_vsw.cell(row=rst_row, column=rst_col + 1, value=base_rst)
    ...
    def generate_sv(self, parsed: Dict[str, Any], context: Dict[str, Any]) -> List[str]:
        ...
        if not blocks:
            return ["// No VSW assertions generated.\n", ""]
        ...
        lines.append("")
        lines.append("int vsw_hsync_cnt;")
        lines.append("")
        lines.append(f"always @(posedge {base_clk} or negedge {base_rst}) begin")
        lines.append(f"    if(!{base_rst}) begin")
        lines.append("        vsw_hsync_cnt <= 0;")
        lines.append("    end")
        lines.append("    else begin")
        lines.append(f"        if({vsync_signal} && $fell({hsync_signal})) begin")
        lines.append("            vsw_hsync_cnt <= vsw_hsync_cnt + 1;")
        lines.append("        end")
        lines.append(f"        else if($fell({hsync_signal})) begin")
        lines.append("            vsw_hsync_cnt <= 0;")
        lines.append("        end")
        lines.append("        else begin")
        lines.append("            vsw_hsync_cnt <= vsw_hsync_cnt;")
        lines.append("        end")
        lines.append("    end")
        lines.append("end")
        lines.append("")
        ...
        lines.append(f"    $fell({vsync_signal}) |-> (({exp_min} <= vsw_hsync_cnt) && (vsw_hsync_cnt <= {exp_max}));")
        lines.append("endproperty")
        lines.append("")
        lines.append("assert property (p_vsw) $display(\"[%0t] Assertion PASS\", $time);")
