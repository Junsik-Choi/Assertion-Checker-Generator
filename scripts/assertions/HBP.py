...
class HBPPlugin(BaseAssertionPlugin):
    ...
    def generate_sv(self, parsed: Dict[str, Any], context: Dict[str, Any]) -> List[str]:
        ...
        lines.append(");")
        lines.append("")
        lines.append("int hbp_low_cnt;")
        lines.append("")
        lines.append(f"always @(posedge {base_clk} or negedge {base_rst}) begin")
        lines.append(f"    if(!{base_rst}) begin")
        lines.append("        hbp_low_cnt <= 0;")
        lines.append("    end")
        lines.append("    else begin")
        lines.append(f"        if({hsync_signal}) begin")
        lines.append("            hbp_low_cnt <= 0;")
        lines.append("        end")
        lines.append("        else begin")
        lines.append("            hbp_low_cnt <= hbp_low_cnt + 1;")
        lines.append("        end")
        lines.append("    end")
        lines.append("end")
        lines.append("")
        ...
        lines.append(f"    $rose({data_enable_signal}) |-> ((hbp_low_cnt >= {exp_min}) && (hbp_low_cnt <= {exp_max}));")
    ...
    def write_to_excel(cls, excel_path: Path, data: Dict[str, Any], state: Optional[Any] = None) -> None:
        """Write HBP assertion data to Excel sheet using labeled row locations."""
        from openpyxl import load_workbook  # type: ignore
        
        try:
            wb = load_workbook(str(excel_path))
            
            # Find HBP sheet (case-insensitive)
            ...
            # Process first block (HBP typically has one block)
            block = blocks[0]
            ...
            raise RuntimeError(f"Failed to write HBP assertion to Excel: {str(e)}")