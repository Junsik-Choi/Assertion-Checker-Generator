...
# ===== 플러그인 =====
@register
class HFPPlugin(BaseAssertionPlugin):
    plugin_name = "hfp"
    sheet_name = "HFP"
    ...
    def parse(self, xls_path: Path) -> Dict[str, Any]:
        ...
        # HFP 시트 확인
        try:
            ws_hfp = _get_sheet_ci(wb_w, self.sheet_name)
        except KeyError:
    ...
    def generate_sv(self, parsed: Dict[str, Any], context: Dict[str, Any]) -> List[str]:
        ...
        lines.append("")
        lines.append("int hfp_clk_cnt;")
        lines.append("logic l_hfp;")
        lines.append("")
        ...
        lines.append(f"        if($fell({data_enable_signal})) begin")
        lines.append("            l_hfp <= 1;")
        lines.append("        end")
        lines.append(f"        else if({hsync_signal}) begin")
        lines.append("            l_hfp <= 0;")
        lines.append("        end")
        lines.append("        else begin")
        lines.append("            l_hfp <= l_hfp;")
        lines.append("        end")
        lines.append("    end")
        lines.append("end")
        lines.append("")
        lines.append(f"always @(posedge {base_clk} or negedge {base_rst}) begin")
        lines.append(f"    if(!{base_rst}) begin")
        lines.append("        hfp_clk_cnt <= 0;")
        lines.append("    end")
        lines.append("    else begin")
        lines.append("        if(l_hfp) begin")
        lines.append("            hfp_clk_cnt <= hfp_clk_cnt + 1;")
        lines.append("        end")
        lines.append(f"        else if(!{hsync_signal}) begin")
        lines.append("            hfp_clk_cnt <= 0;")
        lines.append("        end")
        lines.append("        else begin")
        lines.append("            hfp_clk_cnt <= hfp_clk_cnt;")
        lines.append("        end")
        lines.append("    end")
        lines.append("end")
        lines.append("")
        ...
        lines.append(f"    $fell(l_hfp) |-> (({exp_min} <= hfp_clk_cnt) && (hfp_clk_cnt <= {exp_max}));")
        lines.append("endproperty")
        lines.append("")
        lines.append("assert property (p_hfp) $display(\"[%0t] Assertion PASS\", $time);")
