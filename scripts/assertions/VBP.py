...
# ===== 플러그인 =====
@register
class VBPPlugin(BaseAssertionPlugin):
    plugin_name = "vbp"
    sheet_name = "VBP"
    ...
    def parse(self, xls_path: Path) -> Dict[str, Any]:
        ...
        # VBP 시트 확인
        try:
            ws_vbp = _get_sheet_ci(wb_w, self.sheet_name)
        except KeyError:
        ...
        # 필수 라벨 위치 찾기
        hs_row, hs_col = _find_cell(ws_vbp, "Hsync Signal")
        vs_row, vs_col = _find_cell(ws_vbp, "Vsync Signal")
        de_row, de_col = _find_cell(ws_vbp, "Data Enable Signal")
        min_row, min_col = _find_cell(ws_vbp, "Expected Min Value")
        max_row, max_col = _find_cell(ws_vbp, "Expected Max Value")
        ...
        if (
            ...
            print(
                ...
                "not found in VBP sheet.",
                flush=True,
            )
            raise ValueError("Missing required VBP labels")

        # 기존 시트 값 읽기 (라벨 바로 아래 칸)
        hs_cell  = ws_vbp.cell(row=hs_row  + 1, column=hs_col).value
        vs_cell  = ws_vbp.cell(row=vs_row  + 1, column=vs_col).value
        de_cell  = ws_vbp.cell(row=de_row  + 1, column=de_col).value
        min_cell = ws_vbp.cell(row=min_row + 1, column=min_col).value
        max_cell = ws_vbp.cell(row=max_row + 1, column=max_col).value
        ...
        if hs_cell and str(hs_cell).strip():
            ...
            # 여러 개 후보 → placeholder 두고 메뉴에서 선택
            hsync_signal = "<Hsync Signal>"
        ...
        # 인터랙티브 편집 루프 (HACT/HFP/HSW/VACT와 동일 UX)
        while True:
            print("\n==================== VBP Settings ====================")
        ...
        # 최종 선택 값을 시트에 기록
        ws_vbp.cell(row=hs_row  + 1, column=hs_col,   value=hsync_signal)
        ws_vbp.cell(row=vs_row  + 1, column=vs_col,   value=vsync_signal)
        ws_vbp.cell(row=de_row  + 1, column=de_col,   value=data_enable_signal)
        ws_vbp.cell(row=min_row + 1, column=min_col,  value=exp_min)
        ws_vbp.cell(row=max_row + 1, column=max_col,  value=exp_max)

        # VBP 시트의 Base Clock/Reset 셀에도 Define에서 읽은 값 기록 (수식이 아닌 실제 값)
        clk_row, clk_col = _find_cell(ws_vbp, "Base Clock")
        if clk_row:
            ws_vbp.cell(row=clk_row, column=clk_col + 1, value=base_clk)

        rst_row, rst_col = _find_cell(ws_vbp, "Base Reset")
        if rst_row:
            ws_vbp.cell(row=rst_row, column=rst_col + 1, value=base_rst)
    ...
    def generate_sv(self, parsed: Dict[str, Any], context: Dict[str, Any]) -> List[str]:
        ...
        if not blocks:
            return ["// No VBP assertions generated.\n", ""]
        ...
        lines.append("")
        lines.append("int vbp_hsync_cnt;")
        lines.append("logic l_vbp;")
        lines.append("")
        ...
        lines.append("    end")
        lines.append("    else begin")
        lines.append(f"        if(l_vbp && $rose({hsync_signal})) begin")
        ...
        lines.append("        end")
        lines.append("        else begin")
        lines.append("            vbp_hsync_cnt <= vbp_hsync_cnt;")
        lines.append("        end")
        lines.append("    end")
        lines.append("end")
        lines.append("")
        ...
        lines.append(f"    $fell(l_vbp) |-> (({exp_min} <= vbp_hsync_cnt) && (vbp_hsync_cnt <= {exp_max}));")
        lines.append("endproperty")
        lines.append("")
        lines.append("assert property (p_vbp) $display(\"[%0t] Assertion PASS\", $time);")
