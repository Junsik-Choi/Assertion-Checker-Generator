...
class ClockDividerPlugin(BaseAssertionPlugin):
    ...
    def generate_sv(self, parsed: Dict[str, Any], context: Dict[str, Any]) -> List[str]:
        ...
        lines.append(f"         ##[0:$] $rose({clkout})")
        lines.append(f"         ##0     ({clkout}, high_cnt = high_cnt + 1)[*1:$]")
        lines.append(f"         ##1     $fell({clkout})")
        ...
        lines.append("")
        lines.append("    property p_clkdiv_rose_check0;")
        lines.append(f"        @(posedge {ref_clk} or negedge {ref_clk}) disable iff (!{base_rst} || {disable})")
        lines.append(f"        $rose({start_flag})")
        lines.append(f"        |-> ##[0:MAX*2] $rose({clkout});")
        lines.append("    endproperty")
        lines.append("")
        lines.append("    assert property (p_clkdiv_rose_check0) else `uvm_error(\"ASSERTION\", \"Clock divider check failed\")")
        lines.append("")
