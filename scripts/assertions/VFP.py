...
class VFPPlugin(BaseAssertionPlugin):
    ...
    def parse(self, xls_path: Path) -> Dict[str, Any]:
        ...
        if hs_row is None:
            print("ERROR: 'Hsync Signal' cell not found in VFP sheet.", flush=True)
        ...
        if vs_row is None:
            print("ERROR: 'Vsync Signal' cell not found in VFP sheet.", flush=True)
        ...
        if de_row is None:
            print("ERROR: 'Data Enable Signal' cell not found in VFP sheet.", flush=True)
        ...
        if min_row is None:
            print("ERROR: 'Expected Min Value' cell not found in VFP sheet.", flush=True)
        ...
        if max_row is None:
            print("ERROR: 'Expected Max Value' cell not found in VFP sheet.", flush=True)
    ...
    def generate_sv(self, parsed: Dict[str, Any], context: Dict[str, Any]) -> List[str]:
        ...
        lines.append(f"    $fell(l_vact_vfp) |=> ({exp_min} <= (hsync_cnt - de_cnt) ) && ((hsync_cnt - de_cnt) <= {exp_max});")
