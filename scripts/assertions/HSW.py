    ...
    def parse(self, xls_path: Path) -> Dict[str, Any]:
        ...
        if (
            ...
            print(
                ...
                "not found in HSW sheet.",
                flush=True,
            )
            raise ValueError("Missing required HSW labels")

        # 기존 시트 값 읽기 (라벨 바로 아래 칸)
        hs_cell  = ws_hsw.cell(row=hs_row  + 1, column=hs_col).value
        de_cell  = ws_hsw.cell(row=de_row  + 1, column=de_col).value
        min_cell = ws_hsw.cell(row=min_row + 1, column=min_col).value
        max_cell = ws_hsw.cell(row=max_row + 1, column=max_col).value
