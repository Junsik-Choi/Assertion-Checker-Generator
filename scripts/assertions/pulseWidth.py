...
def _build_hpulse_sv(base_clk: str, base_rst: str, target_pulse: str,
    ...
    lines.append(");")
    lines.append("")
    lines.append("property p_hpulse;")
    ...
    lines.append("    ##1 ({arget_pulse}, value_count = value_count + 1)[*0:$]")
    ...
    lines.append("")
    lines.append(f'assert property (p_hpulse)')
...
def _build_vpulse_sv(base_clk: str, base_rst: str, count_trig: str,
    ...
    lines.append("")
    lines.append("property p_vpulse;")
    lines.append("    int value_count;")
    lines.append(f"    @(posedge {count_trig}) disable iff(!{base_rst})")
    lines.append(f"    ({target_pulse}) |-> (1, value_count = 0)")
    lines.append("    ##0 s_vpulse(value_count)")
    lines.append("    ##1 ({expected_min_value} <= value_count && value_count <= {expected_max_value});")
    lines.append("endproperty")
    lines.append("")
    lines.append(f'assert property (p_vpulse)')
    lines.append('    else $error("failed at %t", $time);')
    lines.append("")
    lines.append("endmodule")
    lines.append("")
