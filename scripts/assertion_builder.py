...
def run_builder(
    ...
    def _run_generation(p_types: List[Type[BaseAssertionPlugin]]):
        for pcls in p_types:
            ...
            try:
                ...
                # VFP, HACT 플러그인은 완전한 interface를 생성하므로 별도 처리
                if pcls.plugin_name in ["vfp", "hact"]:
    ...
    # VFP 또는 HACT 플러그인이 이미 파일을 생성했는지 확인
    intf_generated = ("vfp" in parsed_by_plugin or "hact" in parsed_by_plugin) and gen_sv_path.exists()
    ...
    if not intf_generated:
        ...
        print(f"✓ VFP/HACT plugin already generated {gen_sv_path.name} and {gen_inst_path.name}")
